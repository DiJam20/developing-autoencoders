...
def plot_rfs_for_single_model(model_type: str, model_idx: int = 0, epoch_idx: int = -1) -> None:
    ...
    for i in range(MAX_NEURONS):
        ...
        if i < len(rf_ls):
            plt.imshow(np.transpose(display_ready_rf(rf_ls[i]),(1,2,0)))
        else:
...
def plot_rf_over_time(model_type: str, model_idx: int, neuron_idx: int) -> None:
    ...
    for i in range(len(rf_matrix)):
        axes[i].imshow(np.transpose(display_ready_rf(rf_matrix[i][neuron_idx]),(1,2,0)))
        axes[i].axis('off')
    ...
    for model in range(num_models):
        for epoch in range(num_epochs-1):
            for neuron in range(num_neurons):
                # print('debug', rf_matrices[model][epoch][neuron].shape, rf_matrices[model][epoch + 1][neuron].shape,flush=True)
