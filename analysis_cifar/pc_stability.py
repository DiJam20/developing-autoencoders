...
def calculate_angles_for_single_model(model_idx, model_type, size_ls=None, num_epochs=60):
    ...
    """
    # Load test data
    test_images, _ = initialize_cifar_dataset()
    ...
    for epoch in tqdm(range(num_epochs), desc=f"Model {model_idx} epochs", leave=False):
        ...
        for image in test_images:
            ...
            with torch.no_grad():
                ...
                # print('debug: shape of decoded',encoded.shape,flush=True)
        
        latent_matrix = np.stack(latent_matrix)
        # print('debug: shape of latent_matrix',latent_matrix.shape,flush=True)
        ...
        if model_type == "sae":
            ...
            pca.fit(latent_matrix)
            pca_components = np.pad(pca.components_, (0, 128 - pca.components_.shape[0]), 'constant')
