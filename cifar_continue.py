...
from solver import continue_train_vali_all_epochs, train_vali_all_epochs,nl_dev_train_vali_all_epochs,continue_nl_dev_train_vali_all_epochs, test
...
def continue_train_sae(save_path,n_layers,hidden_ls,n_epochs=10,more_epochs=10):
	sae_model = load_model(sae_path, 'sae', n_layers, hidden_ls, size_ls, n_epochs-1)
	sae_optimizer = torch.optim.SGD(sae_model.parameters(), lr=0.1, momentum=0.9)

	sae_train_loss, sae_validation_loss = continue_train_vali_all_epochs(
		sae_model, 
		train_loader, 
		validation_loader, 
		sae_optimizer, 
...
def continue_train_dae(save_path,n_layers,hidden_ls,size_ls,n_epochs=10, more_epochs=10):
	...
	dae_train_loss, dae_validation_loss = continue_nl_dev_train_vali_all_epochs(
		dae_model, 
		size_ls,
		manner,
		train_loader,
...
if __name__ == '__main__':
	...
	for i in range(num_models):
		...
		more_epochs = 20
		n_epochs = 130
		size_ls = np.ones(more_epochs)*hidden_ls[-1]
		_, dae_train_loss, dae_validation_loss = continue_train_dae(dae_path,n_layers,hidden_ls,size_ls,n_epochs,more_epochs)
		_, sae_train_loss, sae_validation_loss = continue_train_sae(sae_path,n_layers,hidden_ls,n_epochs,more_epochs)
		...
		plt.tight_layout()
		plt.savefig(os.path.join(plot_path, "loss{}.png".format(more_epochs)))
		plt.close(fig)
		...
		plt.savefig(os.path.join(plot_path, "reconstruction_{}.png".format(more_epochs)))
