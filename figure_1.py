...
# PLOT LOSS CURVES
def plot_accuracy_over_epochs(sae_train_loss, dae_train_loss, sae_vali_loss, dae_vali_loss, ax):
    """
    ...
    # Training loss
    sae_train_line, = ax.plot(sae_train_mean, label='AE', color='#1a7adb', linewidth=2)
    ax.fill_between(range(len(sae_train_loss[0])),
                    sae_train_mean - sae_train_std,
                    sae_train_mean + sae_train_std,
                    color='#1a7adb', alpha=0.2)
    
    dae_train_line, = ax.plot(dae_train_mean, label='Dev-AE', color='#e82817', linewidth=2)
    ...
    # Validation loss
    sae_vali_line, = ax.plot(sae_vali_mean, label='AE', color='#1a7adb', linewidth=2, linestyle='--')
    ax.fill_between(range(len(sae_vali_loss[0])),
                    sae_vali_mean - sae_vali_std,
                    sae_vali_mean + sae_vali_std,
                    color='#1a7adb', alpha=0.1)
    
    dae_vali_line, = ax.plot(dae_vali_mean, label='Dev-AE', color='#e82817', linewidth=2, linestyle='--')
    ax.fill_between(range(len(dae_vali_loss[0])),
                    dae_vali_mean - dae_vali_std,
                    dae_vali_mean + dae_vali_std,
                    color='#e82817', alpha=0.1)
    ...
    ax.legend([sae_vali_line, dae_vali_line], ['AE', 'Dev-AE'],
...
def plot_reconstructions(dataset, base_path, iteration, num_examples, ax):
    ...
    grid = gridspec.GridSpecFromSubplotSpec(3, num_examples, subplot_spec=ax.get_subplotspec())
    ...
    for row in range(3):
        for col in range(num_examples):
            ...
            # Row labels
            if col == 0:
                curr_ax.text(-0.3, 0.5, row_labels[row], rotation=90, 
...
def create_combined_figure(sae_train_loss, dae_train_loss, sae_vali_loss, dae_vali_loss, dataset, base_path):
    """
    ...
    plot_accuracy_over_epochs(sae_train_loss, dae_train_loss, sae_vali_loss, dae_vali_loss, ax1)
...
create_combined_figure(sae_train_loss, dae_train_loss, sae_vali_loss, dae_vali_loss, dataset, base_path)